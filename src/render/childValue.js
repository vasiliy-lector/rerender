        ...
        if (type === 'object') {
            if (value.type === 'Template') {
                ...
                for (let i = 0, l = value.length; i < l; i++) {
                    const result = jsx.childValue(value[i],position.updateAbsolute(`${position.id}.${i}`), false);
...
function childValueDom(config, jsx) {
    return function(value, position) {
        if (Array.isArray(value)) {
            ...
            for (let i = 0, l = value.length; i < l; i++) {
                const result = jsx.childValue(value[i],position.updateAbsolute(`${position.id}.${i}`));
                if (Array.isArray(result)) {
                    for (let j = 0, l1 = result.length; j < l1; j++) {
                        memo.push(result[j]);
                    }
